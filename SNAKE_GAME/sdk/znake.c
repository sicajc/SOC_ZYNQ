...
void showWelcome(znake *myZnake){
	// Used to perform down counting on the screen
    char count[10];
	#ifdef whiteMode
	...
	#endif
	char stringSize = 5;
	// The zoom function to zoom in the char, this would make the char on screen a lot larger.
    char zoom=20;
	printString(HSize,VSize,(char *)"Znake",(HSize-stringSize*8*zoom)/2,VSize/3,zoom,redColor,myZnake->Buffer);
	// Down counting from 3,2,1,0
    for(int i=3;i>0;i--){
		sprintf(count, "%d",i);
...
u16 getPreyPosition(znake *myZnake){
	...
	u8 preyX;
	u8 preyY;
    // Since the event timer is always running and changing, changing the srand in all time.
	srand(Xil_In32(myZnake->EventTimerBaseAddress));
    // Generating the random number which is within the board range.
	preyX = rand()%(frameHSize)+frameHOffset+borderWidth;
	preyY = rand()%(frameVSize)+frameVOffset+borderWidth;
    // Concatenating the (x,y) together to allow it to be sent into the HW.
	pos = preyX|(preyY<<8);
	return pos;
}
