...
u32 setupZymouseInterrupt(zyMouse *zyMouseInst, XScuGic *IntcInstancePtr,u32 IRQ){
	u32 Status;
	//XScuGic_SetPriorityTriggerType(IntcInstancePtr,IRQ,0xA0,3); //For proper operation should be level triggered, setting up the priority.
...
static void Mouse_Handler(void *CallBackRef){
	u32 status;
	zyMouse* zyMouseInst;
	// The struct using that pointer, we access the mouse IP.
	zyMouseInst = (zyMouse*)CallBackRef;
	status = Xil_In32(zyMouseInst->baseAddress+IntrStatRegOffset); // Reading from the status regiser.
	xil_printf("%d\n\r",status);
	// Check for the register slots indicating which operation has happened, reading that status register from the IP core.
	if(status&0x1){
		xil_printf("Mouse pressed\n\r");
		zyMouseInst->pressCallBack(zyMouseInst); // Call back function must return by you., there are two callback function
	}
	if(status&0x2){
